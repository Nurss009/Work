...
const Counter = ({ initialCount, handleIncrement, handleDecrement }) => {
...
const Auction = () => {
    const products = [
        ...
        { id: 2, name: 'Держатель для лейки BOOU PG605', price: '999999c', available: true },
    ...
    return (
        <>
            <div className={'container'}>
                <div className="auk">
                    <h1 className={'heat'}>Аукционные товары</h1>
                ...
                <div className="row">
                    {products.map((product) => (
                        <div key={product.id} className="col-2">
                            <img src={shower} alt="" />
                            <h3>{product.name}</h3>
                            <h4 className={'h4-red'}>{product.price} <span className={'sp-12'}>1294</span></h4>
                            ...
                            <Counter initialCount={0} handleIncrement={handleIncrement} handleDecrement={handleDecrement} />
                        </div>
                    ))}
                </div>
            </div>
